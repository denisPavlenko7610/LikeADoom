Shader "NatureManufacture Shaders/Lava/Lava River Tesseled Vertex Color Frozen"
{
	Properties
	{
		...
		_MediumLavaAlbedo_SM("Medium Lava Albedo_SM", 2D) = "bump" {}
		...
		_MediumLavaMT_AO_H_EM("Medium Lava MT_AO_H_EM", 2D) = "bump" {}
		...
		_HotLavaAlbedo_SM("Hot Lava Albedo_SM", 2D) = "bump" {}
		...
		_HotLavaMT_AO_H_EM("Hot Lava MT_AO_H_EM", 2D) = "bump" {}
		...
		_EdgeLength ( "Edge length", Range( 2, 50 ) ) = 9.7
	...
	SubShader
	{
		...
		uniform sampler2D _MediumLavaMT_AO_H_EM;
		uniform half2 _MediumLavaMainSpeed;
		uniform sampler2D _MediumLavaAlbedo_SM;
		uniform float4 _MediumLavaAlbedo_SM_ST;
		...
		uniform sampler2D _HotLavaMT_AO_H_EM;
		uniform half2 _HotLavaMainSpeed;
		uniform sampler2D _HotLavaAlbedo_SM;
		uniform float4 _HotLavaAlbedo_SM_ST;
		...
		uniform sampler2D _MediumLavaNormal;
		uniform half _HotLavaNormalScale;
		uniform sampler2D _HotLavaNormal;
		...
		void vertexDataFunc( inout appdata_full v )
		{
			...
			float2 temp_output_731_0 = (( (float)Direction723 == 1.0 ) ? _ColdLavaMainSpeed :  appendResult705 );
			float2 uv0_ColdLavaAlbedo_SM = v.texcoord.xy * _ColdLavaAlbedo_SM_ST.xy + _ColdLavaAlbedo_SM_ST.zw;
			float2 panner611 = ( _Time.y * temp_output_731_0 + uv0_ColdLavaAlbedo_SM);
			float2 WaterSpeedValueMain614 = panner611;
			float4 break879 = tex2Dlod( _ColdLavaMT_AO_H_EM, half4( WaterSpeedValueMain614, 0, 1.0) );
			...
			float2 uv0_MediumLavaAlbedo_SM = v.texcoord.xy * _MediumLavaAlbedo_SM_ST.xy + _MediumLavaAlbedo_SM_ST.zw;
			float2 panner598 = ( _Time.y * (( (float)Direction723 == 1.0 ) ? _MediumLavaMainSpeed :  appendResult710 ) + uv0_MediumLavaAlbedo_SM);
			float2 SmallCascadeSpeedValueMain600 = panner598;
			float4 break884 = tex2Dlod( _MediumLavaMT_AO_H_EM, half4( SmallCascadeSpeedValueMain600, 0, 1.0) );
			float clampResult890 = clamp( break884.g , ( 1.0 - _MediumLavaAO ) , 1.0 );
			float4 appendResult892 = (half4(( _MediumLavaMetallic * break884.r ) , clampResult890 , pow( ( _MediumLavaEmissionMaskIntesivity * break884.a ) , _MediumLavaEmissionMaskTreshold ) , ( break884.b * _MediumLavaTessScale )));
			float temp_output_932_0 = ( 1.0 - break879.b );
			float temp_output_937_0 = pow( temp_output_932_0 , _MediumLavaHeightBlendTreshold );
			...
			float HeightMask927 = saturate(pow(((temp_output_937_0*clampResult285)*4)+(clampResult285*2),_MediumLavaHeightBlendStrenght));
			...
			float2 uv0_HotLavaAlbedo_SM = v.texcoord.xy * _HotLavaAlbedo_SM_ST.xy + _HotLavaAlbedo_SM_ST.zw;
			float2 panner606 = ( _Time.y * (( (float)Direction723 == 1.0 ) ? _HotLavaMainSpeed :  appendResult712 ) + uv0_HotLavaAlbedo_SM);
			float2 BigCascadeSpeedMain608 = panner606;
			float4 break893 = tex2Dlod( _HotLavaMT_AO_H_EM, half4( BigCascadeSpeedMain608, 0, 1.0) );
			...
			float temp_output_941_0 = pow( temp_output_932_0 , _HotLavaHeightBlendTreshold );
			...
			float HeightMask943 = saturate(pow(((temp_output_941_0*clampResult514)*4)+(clampResult514*2),_HotLavaHeightBlendStrenght));
			float4 lerpResult855 = lerp( lerpResult853 , appendResult898 , HeightMask943);
			float4 break770 = ( v.color / float4( 1,1,1,1 ) );
			float HeightMask988 = saturate(pow(((temp_output_937_0*break770.r)*4)+(break770.r*2),_VCColdLavaHeightBlendStrenght));
			...
			float HeightMask992 = saturate(pow(((break879.b*break770.g)*4)+(break770.g*2),_VCMediumLavaHeightBlendStrenght));
			...
			float HeightMask998 = saturate(pow(((temp_output_941_0*break770.b)*4)+(break770.b*2),_VCHotLavaHeightBlendStrenght));
		...
		void surf( Input i , inout SurfaceOutputStandard o )
		{
			...
			float2 appendResult710 = (half2(_MediumLavaMainSpeed.y , _MediumLavaMainSpeed.x));
			float2 uv0_MediumLavaAlbedo_SM = i.uv_texcoord * _MediumLavaAlbedo_SM_ST.xy + _MediumLavaAlbedo_SM_ST.zw;
			float2 panner598 = ( _Time.y * (( (float)Direction723 == 1.0 ) ? _MediumLavaMainSpeed :  appendResult710 ) + uv0_MediumLavaAlbedo_SM);
			float2 SmallCascadeSpeedValueMain600 = panner598;
			half3 tex2DNode524 = UnpackScaleNormal( tex2D( _MediumLavaNormal, SmallCascadeSpeedValueMain600 ), _MediumLavaNormalScale );
			...
			float2 appendResult712 = (half2(_HotLavaMainSpeed.y , _HotLavaMainSpeed.x));
			float2 uv0_HotLavaAlbedo_SM = i.uv_texcoord * _HotLavaAlbedo_SM_ST.xy + _HotLavaAlbedo_SM_ST.zw;
			float2 panner606 = ( _Time.y * (( (float)Direction723 == 1.0 ) ? _HotLavaMainSpeed :  appendResult712 ) + uv0_HotLavaAlbedo_SM);
			float2 BigCascadeSpeedMain608 = panner606;
			half3 tex2DNode332 = UnpackScaleNormal( tex2D( _HotLavaNormal, BigCascadeSpeedMain608 ), _HotLavaNormalScale );
			float temp_output_941_0 = pow( temp_output_932_0 , _HotLavaHeightBlendTreshold );
			float clampResult507 = clamp( ase_worldNormal.y , 0.0 , 1.0 );
			float temp_output_504_0 = ( _HotLavaAngle / 45.0 );
			float clampResult509 = clamp( ( clampResult507 - ( 1.0 - temp_output_504_0 ) ) , 0.0 , 2.0 );
			...
			float4 break868 = tex2D( _MediumLavaAlbedo_SM, SmallCascadeSpeedValueMain600 );
			...
			float4 break872 = tex2D( _HotLavaAlbedo_SM, BigCascadeSpeedMain608 );
			...
			float clampResult883 = clamp( break879.g , ( 1.0 - _ColdLavaAO ) , 1.0 );
			float4 appendResult876 = (half4(( _ColdLavaMetalic * break879.r ) , clampResult883 , pow( ( _ColdLavaEmissionMaskIntensivity * break879.a ) , _ColdLavaEmissionMaskTreshold ) , ( break879.b * _ColdLavaTessScale )));
			float4 break884 = tex2D( _MediumLavaMT_AO_H_EM, SmallCascadeSpeedValueMain600 );
			float clampResult890 = clamp( break884.g , ( 1.0 - _MediumLavaAO ) , 1.0 );
			float4 appendResult892 = (half4(( _MediumLavaMetallic * break884.r ) , clampResult890 , pow( ( _MediumLavaEmissionMaskIntesivity * break884.a ) , _MediumLavaEmissionMaskTreshold ) , ( break884.b * _MediumLavaTessScale )));
			float4 lerpResult853 = lerp( appendResult876 , appendResult892 , HeightMask927);
			float4 break893 = tex2D( _HotLavaMT_AO_H_EM, BigCascadeSpeedMain608 );
			float clampResult896 = clamp( break893.g , ( 1.0 - _HotLavaAO ) , 1.0 );
			float4 appendResult898 = (half4(( _HotLavaMetallic * break893.r ) , clampResult896 , pow( ( _HotLavaEmissionMaskIntensivity * break893.a ) , _HotLavaEmissionMaskTreshold ) , ( break893.b * _HotLavaTessScale )));
			float4 lerpResult855 = lerp( lerpResult853 , appendResult898 , HeightMask943);
			float4 lerpResult902 = lerp( lerpResult855 , appendResult876 , temp_output_1002_0);
			...
			float temp_output_493_0 = ( pow( min( tex2D( _Noise, ( panner646 + float2( 0.5,0.5 ) ) ).r , tex2D( _Noise, panner321 ).r ) , lerpResult1006 ) * 20.0 );
			float clampResult488 = clamp( temp_output_493_0 , 0.05 , 1.2 );
